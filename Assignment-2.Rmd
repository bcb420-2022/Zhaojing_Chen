...
```
This heat map show there is some separation of signals but is not really clear.
...
DT::datatable(t(broom::tidy(t_test)) ,caption  = "Table 1 : T test result for GR(NR3C1) gene")
```
...
```{r, eval = FALSE, fig.cap='Figure 4:  heatmap for significant differential expression with row normalization'}
...
```{r,fig.cap='Figure 5:unclustered heatmap for significant differential expression after correction',eval=FALSE}
top_hits <- output_hits$ensembl_gene_id[
...
### Q1 
I decided to use the gprofiler2 package for enrichment analysis becuase it would be cohesive with my rest of code and gprofiler2 actually come with some nice graph 
I used Benjamini-Hochberg(BH) for multiple hypothesis testing correction there are thousand of pathway. BH is not as strict as bonferroni so I would not get all pathway out and it corrects false positives.

### Q2
I choose GO biological processes , Reactome and Wikipathway.I choose those becuase I am interested in biological pathway annotation those three are popular and free. I want my initial analysis to be high quality so I exclude electronic version of GO.
...
DT::datatable(df_up_ordered[,c(2,3,4,9,10,11)],caption = "Enrichment analysis for up-regulated genes", options = list(scrollX=T))
```
...
# construct a table about enrichment results
DT::datatable(df_down_ordered[,c(2,3,4,9,10,11)],caption = " Enrichment analysis for down-regulated genes", options = list(scrollX=T))
...
# construct a table about enrichment results
DT::datatable(df_whole_ordered[,c(2,3,4,9,10,11)],caption = " Enrichment analysis for down-regulated genes", options = list(scrollX=T))
...
Because MDA is a stem cell lins. There are some other top hits are related to stem cell characteristics. 
However, because here I just analyze differential expression after the induction of GR not STAT3, My analysis cannot confidently support the final conclusion of the paper that STAT3 and GR coordinates to active  basal-like triple-negative breast cancer.
