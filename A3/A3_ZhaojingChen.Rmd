...
In Assignment 1: Dataset Selection and Initial Processing" and "Assignment 2: Differential Gene Expression and Preliminary Gene Set Enrichment Analysis", can be found using the following links
...
In the second assignment, I performed differential gene expression analysis on data from MDA cell line only, followed by threshold gene enrichment analysis using gprofiler. In this enrichment analysis, I identified key pathways that are enriched from up-regulated expression ,down-regulate gene list and both.

This report performs non-thresholded gene set enrichment analysis on expression data retrieved from GSE152201. The enrichment results are plotted in Enrichment maps. Post analysis are performed against transcription factor dataset retrieve from the baderlab data sets
...
if (!requireNamespace("htmltool",quietly = TRUE)){
...
However, I couldn't figure out when pulling from the docker, how to connect it to my laptop, so most of the code chunk is set not to evaluate.

```{r}
knitr::opts_chunk$set(
  warning = FALSE, message = FALSE,echo = TRUE, tidy = TRUE,eval = FALSE)
```
...
```{r initialize parameters,eval=TRUE}
...
```{r download baderlab gmt file, message=FALSE, warning=FALSE,eval=TRUE, include=FALSE}
...
```{r compute results directory name,eval=TRUE}
...
I summarize our results by displaying the top 5 pathways returned for both treatment and control groups

```{r GSEA results up,eval=TRUE,echo= FALSE,fig.cap="Table 1 : Top 5 GSEA returned pathways in the treatment group, this table describes basic statiscs such as size, enrichemnt scpre, p and q value of the top 5 returned pathways in the treatment group  " }
...
```{r GSEA results down, eval=TRUE,echo= FALSE, fig.cap="Table 2 : Top 5 GSEA returned pathways in the control group, this table describes basic statiscs such as size, enrichemnt scpre, p and q value of the top 5 returned pathways in the control group"   }
...
The top genesetS returned for the up-regulated list is the`glucocorticoid receptor pathway`and the top genesetS returned for the down-regulated list is `Downregulation of TGF-beta receptor signaling`. This result is completely different compared to the top genesets mention above returned by GSEA There are much more genesets returned from the GSEA analysis. While there are a total of 419 genesets using the whole list, there are more than 3000 genesets returned using GSEA

This is not a straightforward comparison. Even though we can see the difference in genesets between the two methods. The parameter settings and the purpose of the two analyses are different. It is hard to compare when the goal of GSEA is to find any possible enrichment geneset while profiler has a much strict threshold and looks for significant genesets
...
```{r installation suammry, fig.cap="Table3: This describes the summary of my list of app to insatlled, whether or not they are succssfully installed"}
...
```{r EM summary,eval= TRUE, fig.cap=" Table 4 : This table describes the basic statistics such as nodes and edges about my initial enrichment map"}
...
tf_annot_png_file_name <- file.path(getwd(),"A3/images","annotation _withtf.png")
...
"Supplementary_Table3_Human_GOBP_AllPathways_no_GO_iea_April_01_2022_symbol.gmt") 
...
gsea_directories <- list.files(path = file.path(getwd(),"A3/data"), 
                                 pattern = "\\.GseaPreranked") 
if(length(gsea_directories) == 1){
  gsea_dir <- file.path(getwd(),"A3/data",gsea_directories[1]) 
...
# na_pos file
enr_file2 <-
read.table(file.path(gsea_dir,gsea_results_files[2]), 
                        header = TRUE, sep = "\t", quote="\"", 
                        stringsAsFactors = FALSE,row.names=1) 
}
...
up <- read.table(file=file.path(getwd(),"data",
                                   "mda_upregulated_genes.txt"),
                                    sep = "\t",
                                    stringsAsFactors = FALSE, ) 
down<- read.table(file=file.path(getwd(),"data",
...
sif_express <-nrow(sig_express_genes)
```
...
sig_genes_no_annotation_enrich <- intersect(sig_express_genes, genes_no_annotation)
...
common <-intersect(sig_genes_no_annotation_pathway,sig_genes_no_annotation_enrich)
...
normalized_count_data<-read.table(file=file.path(getwd(),"Data", "GSE152201_finalized_normalized_counts_mda_2022.txt"),header = TRUE,
                                    sep = "\t")
# select genes that are significant but with no annotation in the entire pathway datset
sig_genes_no_annotation_exp <-normalized_count_data[normalized_count_data$hgnc_symbol %in% sig_genes_no_annotation_enrich,]
...
```{r eval=TRUE,fig.cap="Figure 10 : Heatmap decribing genes that are significant but has no annotation in the enriched genesets"}
...
Since this report is only focusing on GR, I cannot comment about anything related to STAT3. From my analysis, more researches are needed, GR are regulating many of the genes in the significant enriched dataset, so as many other transcription factors. More analysis are needed to uncover how those upreguated genes are related to patience with triple-negative cancer. Comparing to the analysis I did in Assignment 2 where I only looked at threholded results, the GSEA as well as the enrichment map gives a more broad picture. In Assignment, my conclusion was that GR supports the author's conclusion because many of the returend geneset from my up-reguated geneset are related to breast cancer and translation. However, now I knwo that those up-reguated genes are also control by many other transciption facors. Will GR be the master transcription factors regulating all other transcription factor? There is definitely more to inverstgate upon.
